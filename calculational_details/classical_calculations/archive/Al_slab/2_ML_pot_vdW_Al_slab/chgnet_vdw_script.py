...
def relax_slab_chgnet_vdw(slab, fmax=0.05, fix_bottom_layers=2):
    ...
    model = CHGNet.load()
    chgnet_calc = model.get_calculator()
    
    # Set up the D3 calculator for vdW corrections
    d3_calc = DFTD3(dft=chgnet_calc)
    ...
    # Relax the structure
    dyn = BFGS(slab, trajectory='al_slab_relaxation_chgnet_vdw.traj')
    dyn.run(fmax=fmax)
