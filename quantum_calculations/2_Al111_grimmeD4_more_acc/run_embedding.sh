...
docker run --volume-driver local -v $WORK_DIR:/mnt --shm-size=1g --rm --user root cp2k/cp2k sh -c "umask 0000 && mpiexec -genv OMP_NUM_THREADS=2 -np 16 cp2k Al111_active_space.inp" > $WORK_DIR/cp2k.log 2>&1 &
...
python -u client-vqe-ucc.py --nalpha 1 --nbeta 1 --norbs 5 > $WORK_DIR/python_output.log 2>&1 &
